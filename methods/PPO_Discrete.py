...
class PPO_discrete:
    def __init__(self, args):
        ...
        self.initial_clip = args.initial_clip  # 初始 clip 参数
        self.target_clip = args.target_clip  # 目标 clip 参数
        self.decay_steps = args.decay_steps  # 衰减步数

        self.actor = Actor(args)
        self.critic = Critic(args)
        if self.set_adam_eps:  # Trick 9: set Adam epsilon=1e-5
            self.optimizer_actor = torch.optim.Adam(self.actor.parameters(), lr=self.lr_a, eps=1e-5,
                                                    weight_decay=args.l2_reg_factor)
    ...
    def update(self, replay_buffer, total_steps):
        ...
        current_clip = max(self.target_clip,
                           self.initial_clip - (self.initial_clip - self.target_clip) * total_steps / self.decay_steps)
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                surr2 = torch.clamp(ratios, 1 - current_clip, 1 + current_clip) * adv[index]
